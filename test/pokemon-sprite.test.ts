// biome-ignore-all lint/performance/noDelete: TODO: are these `delete`s correct? why are they here?

import fs from "node:fs";
import path from "node:path";
import { beforeAll, describe, expect, it } from "vitest";
import _exp_masterlist from "../images/pokemon/variant/_exp_masterlist.json" with { type: "json" };
import _masterlist from "../images/pokemon/variant/_masterlist.json" with { type: "json" };
...
describe("check if every variant's sprite are correctly set", () => {
  ...
  beforeAll(() => {
    rootDir = `${getAppRootDir()}${path.sep}images${path.sep}pokemon${path.sep}variant${path.sep}`;
